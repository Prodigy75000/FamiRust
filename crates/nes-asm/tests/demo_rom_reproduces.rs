...
//! The committed demo cartridge must be exactly what the committed source
//! assembles to.
//!
...
fn assemble_demo() -> Vec<u8> {
    let src = repo_root().join("roms/famirust-demo/src/main.s");
    let opts = nes_asm::asm::Options { symbol_file: false };
    nes_asm::asm::assemble(&src, &opts)
        .unwrap_or_else(|e| panic!("the demo cartridge no longer assembles:\n{e}"))
        .rom
}

#[test]
fn committed_rom_matches_a_fresh_build_of_the_committed_source() {
    let built = assemble_demo();
    let path = repo_root().join("roms/famirust-demo/famirust-demo.nes");
...
fn assembly_is_deterministic() {
    ...
    assert_eq!(assemble_demo(), assemble_demo());
}

#[test]
fn header_declares_the_cartridge_the_source_asked_for() {
    let rom = assemble_demo();
