# Permission to distribute MONKEY FARCE

**Work:** MONKEY FARCE (`monkey-farce.nes`), a Nintendo Entertainment System /
...
I am the sole author of MONKEY FARCE. I wrote every line of its 6502 assembly
source, drew every graphic in it, designed every room in it, and composed the
music and the sound effects in it. The tune is an original composition; it is
not transcribed, arranged or adapted from any existing piece. It was assembled by `nes-asm`, a 6502 assembler that I also wrote
...
against any existing cartridge. Its character set, its graphics, its levels,
its music and its sounds were made for this cartridge.
...
I dedicate MONKEY FARCE, both the assembled `.nes` image and its complete source,
...
into its output: every byte of `monkey-farce.nes` other than the sixteen-byte iNES
...
That test reassembles `roms/monkey-farce/src/main.s` and compares the result byte
for byte with the committed `monkey-farce.nes`. The published SHA-256 of the
